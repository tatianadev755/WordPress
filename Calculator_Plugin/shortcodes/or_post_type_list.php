...
$readmore_text = !empty($readmore_text)? $readmore_text : __('Read more', 'originbuilder');
...
if ( $the_query->have_posts() ) {
				...
				<?php
					...
					if ( 'post' == get_post_type() && $show_author == 'yes' ) {
						printf( '<span class="author vcard"><i class="sl-user"></i> <a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
							...
							esc_attr( sprintf( __( 'View all posts by %s', 'originbuilder' ), get_the_author() ) ),
							get_the_author()
						);
					}

					$categories_list = get_the_category_list( __( ', ', 'originbuilder' ) );
					if ( $categories_list && $show_category == 'yes') {
						echo '<span class="categories-links"><i class="sl-folder"></i> ' . $categories_list . '</span>';
					}

					$tag_list = get_the_tag_list( '', __( ', ', 'originbuilder' ) );
	...
	echo __('No posts found', 'originbuilder');
}
